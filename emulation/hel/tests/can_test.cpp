...
TEST(CANTest, IDs){
    ...
    std::cout<<"can_motor_controllers:" + hel::to_string(can_motor_controllers, std::function<std::string(std::pair<uint32_t,hel::CANMotorController>)>([&](std::pair<uint32_t, hel::CANMotorController> a){ return "[" + std::to_string(a.first) + ", " + a.second.toString() + "]";}))<<"\n";
    EXPECT_EQ(1, 1); //TODO
