 ...
 * Finds the index of the high bit of the greatest value; returns zero if zero
 * is passed in \tparam T The type of integer \param value The integer to
 * analyze \return The zero-indexed index of the most significant bit
 */
...
template <typename TInteger, typename TIndex,
		  typename = std::enable_if<std::is_integral<TInteger>::value &&
									std::is_integral<TIndex>::value>>
...
struct Maybe {
   ...
   public:
	...
	constexpr bool isValid() const noexcept { return _is_valid; }
 ...
 * \param include_brackets Whether to surround the generated string in brackets
 * or not
 */

template <typename T, typename F>
std::string asString(
	const T& iterable, const F& f, const std::string& delimiter = ",",
	const bool& include_brackets =
		true) {  // TODO use typename Func instead of std::function
	std::string s = "";
	if (include_brackets) {
		s += "[";
	}
 ...
 * \param first_to_s A conversion function from the pair's first value type to a
 * string \param second_to_s A conversion function from the pair's second value
 * type to a string \param delimiter The delimiter to use between the two \param
 * include_brackets Whether to surround the generated string in brackets or not
 */

template <typename First, typename Second>
std::string asString(const std::pair<First, Second>& a,
					 const std::function<std::string(First)>& first_to_s,
					 const std::function<std::string(Second)>& second_to_s,
					 const std::string& delimiter = ",",
					 const bool& include_brackets = true) {
	std::string s = "";
	if (include_brackets) {
		s += "[";
	}
...
constexpr std::size_t hasher(const char* input) {
	return *input ? static_cast<unsigned>(*input) + 33 * hasher(input + 1)
				  : 5381;
}
