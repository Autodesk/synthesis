...
namespace hel{
    ...
    public:
        ...
        constexpr const T& at(std::size_t pos)const{
            if(pos < 0 || pos >= LEN){
                throw std::out_of_range("Synthesis exception: array index out of bounds: index " + std::to_string(pos) + " in array of size " + std::to_string(LEN));
            }
            return internal[pos];
        }
        ...
        constexpr const T& front()const{
            if(LEN == 0){
                throw std::out_of_range("Synthesis exception: array index out of bounds: cannot reference front of array of size " + std::to_string(LEN));
            }
        ...
        constexpr const T& back()const{
            if(LEN == 0){
                throw std::out_of_range("Synthesis exception: array index out of bounds: cannot reference back of array of size " + std::to_string(LEN));
            }
        ...
        BoundsCheckedArray(const S& iterable)noexcept{
            if(iterable.size() != LEN){
                throw std::out_of_range("Synthesis exception: assignment to array of size " + std::to_string(LEN) + " to iterable of different size " + std::to_string(iterable.size()));
            }
        ...
        BoundsCheckedArray(std::initializer_list<T> list){
            if(list.size() != LEN){
                throw std::out_of_range("Synthesis exception: assignment to array of size " + std::to_string(LEN) + " to brace-enclosed initializer list of different size " + std::to_string(list.size()));
            }
