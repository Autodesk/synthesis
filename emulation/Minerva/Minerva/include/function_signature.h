...
/**
 * \brief The class FunctionSignature represents information about a function
...
struct FunctionSignature {
	...
	struct ParseException
		: public std::exception {  // TODO: this could probably be improved
		const char* what() const throw() { return "Parsing failed"; }
	};

	/**
	 * \brief The class ParameterNameInfo represents information about a
	 * variable declaration in string form
	 *
	...
	/**
	 * \brief The class ParameterValueInfo represents information about a
	 * variable's value in string form
	 *
	 * Stores its type and its value in string form - currently only supports
	 * types covered by minerva::HALType
	 */
	...
	// These constants are used to parse function signatures for the RoboRIO HAL
	// which follow a consistent naming scheme
	 ...
	 *
	 * Assumes the function signature's return type and name are all on the same
	 * line, though its parameters may span multiple lines after that
	 *
	 * @param The standard string to parseFunctionSignature
	 * @return A FunctionSignature representing the parsed line
	 * @throw ParseException if it is unable to parse the string due to
 ...
 * @return A standard vector of FunctionSignatures representing the parsed
 * functions in the header file
 */
std::vector<FunctionSignature> parseFunctionSignatures(const std::string);

std::ostream& operator<<(std::ostream&,
						 const minerva::FunctionSignature::ParameterNameInfo);
bool operator<(const minerva::FunctionSignature::ParameterNameInfo,
			   const minerva::FunctionSignature::ParameterNameInfo);
std::ostream& operator<<(std::ostream&,
						 minerva::FunctionSignature::ParameterValueInfo);
std::ostream& operator<<(std::ostream&, const minerva::FunctionSignature);
bool operator<(const minerva::FunctionSignature,
			   const minerva::FunctionSignature);
