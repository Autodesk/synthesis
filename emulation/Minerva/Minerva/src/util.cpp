...
string minerva::excludeFromString(string input_str,
								  vector<char> excluded_chars) {
...
vector<string> minerva::split(string input_str, const char DELIMITER) {
	...
	while (input_str.find(DELIMITER) != string::npos) {
		...
		input_str = input_str.substr(segment.size() +
									 1);  // remove the segment added to the
										  // vector along with the delimiter
	}
