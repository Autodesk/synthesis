    ...
    {	
        if (activeRobot != null && skeleton != null)
        {
            ...
            if (unityWheelData.Count > 0)
            {
				...
				while(auxFunctions.rightRobot(unityWheelData, activeRobot.transform)&& counter < 3);
            }
        }
                ...
                if (uNode.GetSkeletalJoint() != null)
                {
                    if (uNode.GetSkeletalJoint().GetJointType() == SkeletalJointType.LINEAR)
                    {
                        ...
                        if (Input.anyKey)
                        {
                            ...
                            int speedArrowPWM = 1;
                            pwm[3] +=
                                (Input.GetKey(KeyCode.Alpha1) ? speedArrowPWM : 0.0f);
                            pwm[3] +=
                                (Input.GetKey(KeyCode.Alpha0) ? -speedArrowPWM : 0.0f);
                        }
						if(es != null)
						{
							for(int i = 0; i < 8; i++)
