...
namespace Synthesis.DriverPractice
{
    ...
    public class DriverPracticeRobot : LinkedMonoBehaviour<MainState>
    {
        ...
        {
            for (int i = 0; i < Controls.Players[controlIndex].buttons.pickup.Count; i++) //for each gamepiece
                if (DPMDataHandler.dpmodes.Where(d => d.gamepiece.Equals(FieldDataHandler.gamepieces[i].name)).ToArray().Count() > 0 && //existence check
                     InputControl.GetButton(Controls.Players[controlIndex].buttons.pickup[i])) Intake(i);
            for (int i = 0; i < Controls.Players[controlIndex].buttons.release.Count; i++)
                if (DPMDataHandler.dpmodes.Where(d => d.gamepiece.Equals(FieldDataHandler.gamepieces[i].name)).ToArray().Count() > 0)
                    if (InputControl.GetButton(Controls.Players[controlIndex].buttons.release[i])) Release(i);
                    else HoldGamepiece(i); //when not releasing hold
            for (int i = 0; i < Controls.Players[controlIndex].buttons.spawnPieces.Count; i++)
                if (DPMDataHandler.dpmodes.Where(d => d.gamepiece.Equals(FieldDataHandler.gamepieces[i].name)).ToArray().Count() > 0 &&
                    InputControl.GetButtonDown(Controls.Players[controlIndex].buttons.spawnPieces[i])) Spawn(FieldDataHandler.gamepieces[i]);
            if (InputControl.GetButtonDown(Controls.Players[controlIndex].buttons.trajectory)) drawing = !drawing; //trajectory drawing toggle
        }
