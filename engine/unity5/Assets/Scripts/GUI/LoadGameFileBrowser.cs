...
/// </summary>
class LoadGameFileBrowser : OverlayWindow
{
    ...
    public LoadGameFileBrowser(string windowTitle, bool allowEsc = true)
    {
        Init(windowTitle, Directory.GetParent(Application.dataPath).FullName, allowEsc);
    }

    public LoadGameFileBrowser(string windowTitle, string defaultDirectory, bool allowEsc = true)
    {
    ...
    private void FileBrowserWindow(int idx)
    {
        ...
        // Get the directory info of the current location
        {
            ...
            if ((fileSelection.Attributes & FileAttributes.Directory) == FileAttributes.Directory)
            {
                ...
                if (directoryInfo.GetDirectories().Length == 0 && title.Equals("Load Save"))
                {
        ...
        GUILayout.Label("When choosing a file, please select a .csv save file" +
                " " +  descriptionStyle);
