...
namespace Synthesis.Import {
    public static class MirabufDefinitionHelper {
#region Creation

        /// <summary>Generates the unity gameobjects for a mix and match robot and merges all parts together</summary>
        public static (Dictionary<string, GameObject> objectDict,
            Matrix4x4 transformation)[] GenerateMixAndMatchDefinitionObjects(MirabufLive[] miraLiveFiles,
            ...
            int dynamicLayer = DynamicLayerManager.NextRobotLayer;

            // Generates the unity gameobjects with components for each part
            miraLiveFiles.ForEachIndex((i, m) => groupObjects[i] = m.GenerateDefinitionObjects(
                                           assemblyContainer, true, true, true, i, dynamicLayer));

            // Generates part transformations
            ...
            // Creates a new grounded object that all part grounded objects will be combined onto
            ...
            groupObjects.ForEachIndex((i, objects) => {
                ...
                GameObject parentNode;
                {
                    ...
                    var obj        = groupObjects[parentPartIndex].Values.Find(x => x.name == targetName);
            ...
            PartFound:
                ...
                // Sets the parent node as the part's grounded object to attach joints to
                if (objects.TryGetValue("grounded", out var partGrounded)) {
                    ...
                    Object.DestroyImmediate(partGrounded);

                    objects.Add("grounded", parentNode);
                } else {
                    Logger.Log($"No grounded object found on part {robotTransformData.GlobalPartData[i].Name}");
                }
            });

            // Divides by total robot mass to get the final CoM of the whole robot
            rb.centerOfMass /= rb.mass;

            return groupObjects.Zip(transformations, (d, t) => (d, t)).ToArray();
        }

        public enum ColliderGenType {
            NoCollider = 0,
            Convex     = 1,
            Concave    = 2
        }
...
#region Rigidbody Identification
        ...
        /// </summary>
        public static MirabufLive.RigidbodyDefinitions FindRigidbodyDefinitions(MirabufLive live) {
            ...
            var defs    = new Dictionary<string, MirabufLive.RigidbodyDefinition>();
            ...
            // current scope
            Action<MirabufLive.RigidbodyDefinition, MirabufLive.RigidbodyDefinition> MergeDefinitions =
                (keep, delete) => {
            ...
            foreach (var jInst in assembly.Data.Joints.JointInstances) {
                ...
                var mainDef        = new MirabufLive.RigidbodyDefinition { GUID = $"{counter}",
                    Name  = $"{jInst.Value.Info.Name}_Rigidgroup{counter}",
                    Parts = new Dictionary<string, PartInstance>() };
                defs[mainDef.GUID] = mainDef;

                var stubDef        = new MirabufLive.RigidbodyDefinition { GUID = $"{counter}-2",
                ...
                if (jInst.Value.Parts is { Nodes : { Count : > 0 } }) {
                ...
                // Grab all parts eligible via design hierarchy
                ...
                void Inherit(string part, string definition) {
                    ...
                    var originalDef = partMap.TryGetValue(part, out var value) ? value : string.Empty;
                    ...
                    while (toCheck.Count > 0) {
                        toCheck.ForEach(y => {
                            ...
                            if (existingDef == null) {
                                ...
                                tmp.AddRange(y.Children.Where(z => z is { Value : not null })); // Just to make sure
            ...
            MirabufLive.RigidbodyDefinition groundDef = new MirabufLive.RigidbodyDefinition { Name = "grounded",
                GUID = "grounded", Parts = new Dictionary<string, PartInstance>() };
            defs.Add(groundDef.GUID, groundDef);
            groundJoint.joint.Parts?.Nodes?.AllTreeElements().Where(x => !string.IsNullOrEmpty(x.Value)).ForEach(x => {
            ...
            discoveredRigidGroups.Where(x => x.Occurrences.Count > 1).ForEach(x => {
                MirabufLive.RigidbodyDefinition rigidDef = new MirabufLive.RigidbodyDefinition { Name = $"{x.Name}",
            ...
            MirabufLive.RigidbodyDefinition newGrounded = default;
                ...
                assembly.Data.Parts.PartDefinitions.Where(x => x.Value.Dynamic)
                    .ForEach(
                        x => assembly.Data.Parts.PartInstances.Where(y => y.Value.PartDefinitionReference.Equals(x.Key))
                                 .ForEach(y => {
                                     MirabufLive.RigidbodyDefinition rigidDef =
                                         new MirabufLive.RigidbodyDefinition { Name = $"gamepiece_{gamepieceCounter}",
                                             GUID = $"{counter}", Parts = new Dictionary<string, PartInstance>() };
                                     defs.Add(rigidDef.GUID, rigidDef);
            ...
            foreach (var group in defs.Values.Select(x => x.GUID)) {
                var collectivePhysData = new List<(Matrix4x4, PhysicalProperties)>();
            ...
            var definitions = new MirabufLive.RigidbodyDefinitions { Mass = totalMass, Definitions = defs,
        ...
        private static List<string> GetAllPartsInBranch(
            ...
            var toCheck = new List<Node> { NavigateDHPath(paths[rootPart].Append(rootPart).ToList(), rootNode) };
            while (toCheck.Count > 0) {
...
#region Global Transformations
        ...
        private static void MakeGlobalTransformations(Assembly assembly) {
