...
namespace Synthesis.Import {
    public static class Importer {
        ...
        public static (GameObject mainObject, MirabufLive[] miraLiveFiles, FieldSimObject sim) ImportField(
            string filePath) {
            ImportHelper importHelper = new ImportHelper(filePath);
        ...
        private class ImportHelper {
            ...
            public ImportHelper(MixAndMatchRobotData mixAndMatchRobotData) : this(mixAndMatchRobotData,
                mixAndMatchRobotData.PartData
                    .Select(part => new MirabufLive(MixAndMatchSaveUtil
                        .LoadPartData(part.fileName).MirabufPartFile)).ToArray()) { }

            public ImportHelper(string filePath) : this(null,
                new[] { new MirabufLive(filePath) }) { }
            ...
            public ImportHelper(MixAndMatchRobotData mixAndMatchRobotData, MirabufLive[] miraLiveFiles) {
                ...
                _groupObjects = (mixAndMatchRobotData == null)
                    ...
                    : MirabufLive.GenerateMixAndMatchDefinitionObjects(_miraLiveFiles, _assemblyObject,
                        mixAndMatchRobotData);
            }
            ...
            public void ConfigureGamepieces() {
                _groupObjects.ForEachIndex(
                    (i, x) => x.Where(x => _miraLiveFiles[i].Definitions.Definitions[x.Key].IsGamepiece).ForEach(x => {
                        var gpSim = new GamepieceSimObject(_miraLiveFiles[i].Definitions.Definitions[x.Key].Name,
                            x.Value);
                        try {
            ...
            public void CreateSimObject() {
                var state = _assemblies[0].Data.Signals == null
            ...
            public void MakeAllJoints() {
                ...
                _assemblies.ForEachIndex((partIndex, assembly) => assembly.Data.Joints.JointInstances.ForEach(
                    jointKvp => {
            ...
            private void MakeJoint(GameObject gameObjectA, GameObject gameObjectB, JointInstance instance,
                ...
                void WheelJoint() {
                    ...
                    if (instance.HasSignal()) {
                        var driver = new WheelDriver(
                            ...
                            new []
                                { $"{instance.SignalReference}_encoder", $"{instance.SignalReference}_absolute" },
                ...
                void HingeJoint() {
                    ...
                    if (limits != null && limits.Lower != limits.Upper) {
                        ...
                        revoluteA.limits =
                            new JointLimits()
                                { min = -limits.Upper * Mathf.Rad2Deg, max = -limits.Lower * Mathf.Rad2Deg };
                    }
                    ...
                    if (instance.HasSignal()) {
                        var slideDriver =
                            new LinearDriver(assembly.Data.Signals.SignalMap[instance.SignalReference].Info.GUID,
                                new [] { instance.SignalReference }, Array.Empty<string>(), _simObject, sliderA,
                                sliderB, 0.1f,
