...
namespace Synthesis.Configuration {
    public class MoveArrow : MonoBehaviour {
        ...
        private float singleMoveLimitScale = 20f;
        ...
        // Called to set certain value when activated or when the parent changes
        ...
        // Makes sure values are set correctly when the gizmo is removed
        ...
        private void Update() {
            ...
            // Skip if there no gizmo components being dragged
            ...
            if (activeArrow == ArrowType.P) {
                ...
                if (activeArrow <= ArrowType.Z) {
                    ...
                    // Limits the maximum distance the ray can be to prevent moving it to infinity
                    ...
                    // Limits ray distance to prevent moving to infinity
                ...
                if (lastArrowPoint != Vector3.zero) {
                    // Snaps to configurable amount when control is held down. Does this
                    // by setting current arrow point to rounded distance
                    if (snapEnabled && activeArrow <= ArrowType.Z) {
                    ...
                    setLastArrowPoint         = projectedPosition.y >= floorBound; // sets movement boundaries
                    if (setLastArrowPoint) {
                        parent.position += currentArrowPoint - lastArrowPoint;
                    }
                }

                // last arrow point keeps track of where the mouse is relative
                ...
                parent.RotateAround(parent.position, axisPlane.normal,
                    -1 *
                        RoundTo(Vector3.SignedAngle(pointToLook - parent.position, t, axisPlane.normal) - startRotation,
                            snapEnabled ? snapRotationToDegree : 0f));
            }
        ...
        private void OnArrowSelected(ArrowType arrowType) {
            ...
            if (arrowType == ArrowType.P) {
                ...
                // Sets up axis arrows for plane creation
                switch (arrowType) {
                    case ArrowType.X:
        ...
        // Save the state of all gameobject's rigidbodies as a dictionary
        public void HierarchyRigidbodiesToDictionary() {
            rigidbodyKinematicStateInScene = new Dictionary<Rigidbody, bool>();
            GameObject Game                = GameObject.Find("Game");

            // Searches for all rigidbodies under the "Game" parent
            foreach (Rigidbody rb in Game.GetComponentsInChildren<Rigidbody>()) {
                // skips gizmos
                if (rb.gameObject.transform.parent != transform) {
                    rigidbodyKinematicStateInScene.Add(rb, rb.isKinematic);
                }
            }
        }
        ...
        // Used for snapping the gizmo to the nearest value
