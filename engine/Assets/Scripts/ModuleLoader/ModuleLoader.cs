    ...
    public static class ModuleLoader {
        ...
        public static void LoadModules(string moduleSourcePath, string baseModuleTargetPath) {
            ...
            foreach (var (archive, metadata) in modules) {
                try {
                    ...
                    ModuleManager.AddToLoadedModuleList(new ModuleManager.ModuleInfo(
                        metadata.Name, metadata.Version, metadata.Author, metadata.Description));
                } catch (Exception e) {
                ...
                foreach (var (_, metadata) in modules) {
                    if (metadata.Name == module?.Item2.Name) {
                        ...
                        throw new LoadModuleException(
                            $"Attempting to load module with duplicate name: {metadata.Name}");
                    }
                    if (metadata.TargetPath == module?.Item2.TargetPath) {
                        ...
                        throw new LoadModuleException(
                            $"Attempting to load modules into same target path: {metadata.TargetPath}");
                    }
                }
        ...
        private static PreloadedModule? PreloadModule(string filePath) {
            ...
            try {
                ...
                var metadata = ModuleMetadata.Deserialize(
                    module.Entries.First(e => e.FileName == ModuleMetadata.MetadataFilename).OpenReader());
        ...
        public static void ResolveDependencies(List<(ZipFile archive, ModuleMetadata metadata)> moduleList) {
            foreach (var (_, metadata) in moduleList) {
                foreach (var dependency in metadata.Dependencies) {
                    if (moduleList.All(m => m.metadata.Name != dependency.Name)) {
                        throw new LoadModuleException(
                            $"Module {metadata.Name} is missing dependency module {dependency.Name}");
                    }
                    var present_dep = moduleList.First(m => m.metadata.Name == dependency.Name);
                    if (present_dep.metadata.Version != dependency.Version) {
                        throw new LoadModuleException(
                            $"Module {metadata.Name} requires dependency module {dependency.Name} version {dependency.Version} but its version is {present_dep.metadata.Version}");
                    }
                }
            }
            ...
            while (resolvedEntries.Count > 0) {
                ...
                foreach (var dep in moduleList
                             .Where(t => t.metadata.Dependencies.Any(
                                        d => d.Name == element.metadata.Name && d.Version == element.metadata.Version))
                             .ToList()) {
                    dep.metadata.Dependencies.RemoveAll(
                        d => d.Name == element.metadata.Name && d.Version == element.metadata.Version);
        ...
        private static string GetPath(string fullName) {
            var i = fullName.LastIndexOf(SynthesisAPI.VirtualFileSystem.Directory.DirectorySeparatorChar,
                fullName.Length - 1, fullName.Length - 2);
        ...
        public static void LoadModule((ZipFile archive, ModuleMetadata metadata) moduleInfo) {
            ...
            var metadataPath =
                GetPath(moduleInfo.archive.Entries.First(e => e.FileName == ModuleMetadata.MetadataFilename).FileName);

            List<(Assembly assembly, string owningModule)> loadedAssemblies =
                new List<(Assembly assembly, string owningModule)>();

            foreach (var entry in moduleInfo.archive.Entries.Where(e => {
                         var name = RemovePath(metadataPath, e.FileName);
                         return name != ModuleMetadata.MetadataFilename &&
                                moduleInfo.metadata.FileManifest.Contains(name);
                     })) {
                ...
                if (extension == ".dll") {
                    ...
                    var targetPath = _baseModuleTargetPath +
                                     ...
                                     moduleInfo.metadata.TargetPath +
                                     SynthesisAPI.VirtualFileSystem.Directory.DirectorySeparatorChar +
                    ...
                    if (type == null) {
                        throw new LoadModuleException(
                            $"Failed to determine asset type from file extension of asset: {entry.FileName}");
                    } else if (AssetManager.Import(type, new DeflateStreamWrapper(stream, entry.UncompressedSize),
                                   targetPath, entry.FileName, perm, "") == null) {
            ...
            foreach (var file in fileManifest) {
                Logger.Log(
                    $"Module \"{moduleInfo.metadata.Name}\" is missing file from manifest: {file}", LogLevel.Warning);
            }
        }
            ...
            try {
                ...
                var types =
                    apiAssembly.GetTypes()
                        .Where(
                            t => t.GetCustomAttribute<ModuleOmitAttribute>() == null &&
        ...
        public static bool LoadModuleAssembly(
            Stream stream, string owningModule, List<(Assembly assembly, string owningModule)> loadedAssemblies) {
                ...
                try {
                    if (initializer != null) {
                        ...
                        if (init != null) {
                            ...
                            Logger.Log($"Initializer for module {owningModule} does not have a valid Initialize method",
                                LogLevel.Warning);
                        }
                    } else if (system != null) {
                        var entity        = EnvironmentManager.AddEntity();
                    ...
                    Logger.Log(
                        $"Module loader failed to process type {errorSource} from module {owningModule}\nType: {e.TypeName}, Site: {e.TargetSite}\n{e}",
                        LogLevel.Error);
                } catch (Exception e) {
                    var errorSource = system == null ? callback.DeclaringType : system;
        ...
        private static bool ProcessLoadedAssemblies(List<(Assembly assembly, string owningModule)> assemblies) {
            foreach (var loadedAssembly in assemblies) {
                            ...
                            t => t.GetCustomAttribute<ModuleOmitAttribute>() == null &&
                                 (t.IsSubclassOf(typeof(SystemBase)) ||
                                     t.GetMethods().Any(m => m.GetCustomAttribute<CallbackAttribute>() != null ||
                                                             m.GetCustomAttribute<TaggedCallbackAttribute>() != null)))
                        .ToList();
                var prioritizedInitializations =
                    ResolveInitializationOrder(exportedModuleClasses, loadedAssembly.owningModule);
                var instances = new Dictionary<Type, object>();
                    ...
                    try {
                        if (initializer != null) {
                            ...
                            if (init != null) {
                                ...
                                Logger.Log(
                                    $"Initializer for module {loadedAssembly.owningModule} does not have a valid Initialize method",
                                    LogLevel.Warning);
                            }
                        } else if (system != null) {
                            var entity        = EnvironmentManager.AddEntity();
                        ...
                        var errorSource = system == null ? callback.DeclaringType : system;
                        Logger.Log(
                            $"Module loader failed to process type {errorSource} from module {loadedAssembly.owningModule}\nType: {e.TypeName}, Site: {e.TargetSite}\n{e}",
                            LogLevel.Error);
                    } catch (Exception e) {
                        var errorSource = system == null ? callback.DeclaringType : system;
                        Logger.Log(
                ...
                ModuleManager.RegisterModuleAssemblyName(
                    loadedAssembly.assembly.GetName().Name, loadedAssembly.owningModule);
            }
            return true;
        }

        private static List<(Type initializer, Type system, MethodInfo callback)> ResolveInitializationOrder(
            List<Type> types, string moduleName) {
            var systems = types.Where(t => t.IsSubclassOf(typeof(SystemBase))).ToList();
            var callbacks =
                types
                    .Where(t => !t.IsSubclassOf(typeof(SystemBase)) ||
                                t.GetMethods().Any(m => m.GetCustomAttribute<CallbackAttribute>() != null ||
                                                        m.GetCustomAttribute<TaggedCallbackAttribute>() != null))
                    .SelectMany(
                        m => m.GetMethods().Where(m2 => m2.GetCustomAttribute<CallbackAttribute>() != null ||
                                                        m2.GetCustomAttribute<TaggedCallbackAttribute>() != null))
                    .ToList();
            var prioritizedSystems = systems.Where(t => t.GetCustomAttribute<InitializationPriorityAttribute>() != null)
                                         .OrderBy(t => t.GetCustomAttribute<InitializationPriorityAttribute>().Value)
                                         .ToList();
            var prioritizedCallbacks =
                callbacks.Where(m => m.GetCustomAttribute<InitializationPriorityAttribute>() != null)
                    .OrderBy(m => m.GetCustomAttribute<InitializationPriorityAttribute>().Value)
                    .ToList();
            ...
            foreach (var system in systems.Where(
                         t => t.GetCustomAttribute<InitializationPriorityAttribute>() == null)) {
                prioritizedElements.Add((null, system, null));
            }

            foreach (var callback in callbacks.Where(
                         m => m.GetCustomAttribute<InitializationPriorityAttribute>() == null)) {
        ...
        private static void RegisterTagCallback(MethodInfo callback, object instance) {
            ...
            typeof(EventBus)
                .GetMethod("NewTagListener")
                ?.Invoke(null, new object[] { tag, CreateEventCallback(callback, instance, eventType) });
        }
        ...
        private static void RegisterTypeCallback(MethodInfo callback, object instance) {
            ...
            var eventType = callback.GetParameters().First().ParameterType;
            typeof(EventBus)
                .GetMethod("NewTypeListener")
                ?.MakeGenericMethod(eventType)
                .Invoke(null, new object[] { CreateEventCallback(callback, instance, eventType) });
        }

        private static EventBus.EventCallback CreateEventCallback(MethodInfo m, object instance, Type eventType) {
            return (e) => m.Invoke(instance, new[] { typeof(ReflectHelper)
                                                         .GetMethod("CastObject")
                                                         ?.MakeGenericMethod(eventType)
