    ...
    public class SelectableAdapter : MonoBehaviour, IApiAdapter<Selectable> {
        ...
        private void Select() {
            ...
            if (!instance.IsSelected) {
                Deselect();
                instance.SetSelected((currentClickTime - lastClickTime) <= 400
                                         ? Selectable.SelectionType.ExtendedSelection
                                         : Selectable.SelectionType.ExtendedSelectionPending);
                instance.OnSelect();
        ...
        public void OnEnable() {
            ...
            if (selectableAdapterCount == 0) {
                InputManager.AssignDigitalInput(
                    $"_internal SelectableAdapter deselect", new Digital($"mouse 1 non-ui"), e => {
                        if (Selectable.Selected != null) {
                            var lastSelected = Selectable.Selected;
                            Deselect();
                            EventBus.Push(new Selectable.SelectionChangeEvent(Selectable.Selected, lastSelected));
            ...
            if (mouseDownEvent.State == DigitalState.Down) {
                ...
                foreach (var hit in hits) {
                    if (hit.collider.transform == transform) {
                        hitMe = true;
                    } else if (ApiProviderData.GameObjects.TryGetValue(
                                   hit.collider.transform.gameObject, out Entity otherE)) {
