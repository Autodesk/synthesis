...
public class DraggablePanel : MonoBehaviour, IDragHandler, IBeginDragHandler
{
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        if (lastRectPosition.y > dh - h) lastRectPosition = new Vector3(lastRectPosition.x, dh-h);
        else if (lastRectPosition.y < h) lastRectPosition = new Vector3(lastRectPosition.x, h);

        float w = parent.rect.width / 2;
        float dw = draggableArea.rect.width;
        if (lastRectPosition.x > dw-w) lastRectPosition = new Vector3(dw - w,lastRectPosition.y);
        else if (lastRectPosition.x < w) lastRectPosition = new Vector3(w, lastRectPosition.y);
