...
namespace Synthesis.Util {
    public static class Toaster {
        ...
        public const float TOAST_CONTAINER_HEIGHT =
            TOAST_HEIGHT * (MAX_SHOWN_TOASTS + 1) + TOAST_SPACING * MAX_SHOWN_TOASTS;
        public static readonly Color DROP_SHADOW_MOD_COLOR = new Color(0.3f, 0.3f, 0.3f, 0.8f);

        public static readonly(Color32 left, Color32 right) TOAST_DEBUG_COLORS   = (new Color32(55, 34, 188, 255),
            new Color32(132, 25, 216, 255));
        public static readonly(Color32 left, Color32 right) TOAST_INFO_COLORS    = (new Color32(78, 121, 228, 255),
            new Color32(76, 164, 192, 255));
        public static readonly(Color32 left, Color32 right) TOAST_WARNING_COLORS = (new Color32(223, 123, 30, 255),
            new Color32(212, 216, 25, 255));
        public static readonly(Color32 left, Color32 right) TOAST_ERROR_COLORS   = (new Color32(175, 14, 33, 255),
            new Color32(228, 59, 59, 255));
    ...
    public class ToastHandler : IDisposable {
        ...
        public ToastHandler(string message, string? title = null, LogLevel level = LogLevel.Debug,
            ...
            _toastMainContent = DynamicUIManager.ToastContainer
                                    .CreateSubContent(new Vector2(Toaster.TOAST_CONTAINER_WIDTH,
                                        Toaster.TOAST_HEIGHT + Toaster.DROP_SHADOW_DISTANCE))
                                    .SetBottomStretch<Content>();
            var dropShadow =
                _toastMainContent.CreateSubContent(new Vector2(Toaster.TOAST_CONTAINER_WIDTH, Toaster.TOAST_HEIGHT))
                    .SetStretch<Content>(topPadding: Toaster.DROP_SHADOW_DISTANCE)
                    .EnsureImage();
            var content =
                _toastMainContent.CreateSubContent(new Vector2(Toaster.TOAST_CONTAINER_WIDTH, Toaster.TOAST_HEIGHT))
                    .SetStretch<Content>(bottomPadding: Toaster.DROP_SHADOW_DISTANCE)
                    .EnsureImage();
            ...
            content.StepIntoImage(i => i.SetSprite(null).SetColor(colors.left, colors.right).SetCornerRadius(15f));
            dropShadow.StepIntoImage(i => i.SetSprite(null)
                                              .SetColor(colors.left * Toaster.DROP_SHADOW_MOD_COLOR,
            ...
            var infoContent =
                content
                    ...
                    .SetStretch<Content>(topPadding: infoContentPadding.top, bottomPadding: infoContentPadding.bottom,
                        leftPadding: infoContentPadding.left, rightPadding: infoContentPadding.right);
            var (iconContent, textButtonContent) = infoContent.SplitLeftRight(
                Toaster.TOAST_HEIGHT - (infoContentPadding.top + infoContentPadding.bottom), 15f);

            var buttonContentPadding = 10f;
            var buttonContentWidth   = (textButtonContent.Size.y - buttonContentPadding) / 2f;
            var (textContent, buttonContent) =
                textButtonContent.SplitLeftRight(textButtonContent.Size.x - (15f + buttonContentWidth), 15f);

            var (topButtonContent, bottomButtonContent) =
                buttonContent.SplitTopBottom(buttonContent.Size.x, buttonContentPadding);
            topButtonContent.CreateButton("")
                .SetStretch<Button>(topPadding: -5f, bottomPadding: -5f, rightPadding: -5f, leftPadding: -5f)
                .StepIntoImage(
                    i => i.SetSprite(SynthesisAssetCollection.GetSpriteByName("CloseIcon")).SetColor(Color.white))
                .AddOnClickedEvent(
                    _ => {
            ...
            if (optionalCallback != null) {
                bottomButtonContent.CreateButton("")
                    .SetStretch<Button>(topPadding: -5f, bottomPadding: -5f, rightPadding: -5f, leftPadding: -5f)
                    .StepIntoImage(i => i.SetSprite(SynthesisAssetCollection.GetSpriteByName("DownArrowIcon"))
                                            .SetColor(Color.white))
                    .AddOnClickedEvent(
                        _ => { optionalCallback.Invoke(this); });
            }

            var (topLabelContent, bottomLabelContent) =
                textContent.SplitTopBottom(Toaster.TOAST_HEIGHT / 2 - (infoContentPadding.top + 3.75f), 7.5f);

            if (s != null)
                iconContent.EnsureImage().StepIntoImage(i => i.SetSprite(s).SetColor(Color.white));

            topLabelContent.CreateLabel().SetStretch().SetText(title).SetAutomaticFontSize(true).SetColor(Color.white);
            bottomLabelContent.CreateLabel()
                .SetStretch(topPadding: -3f, bottomPadding: -3f)
                .SetText(message)
                .SetAutomaticFontSize(true)
        ...
        public void SetHeight(float height) {
            ...
            if (Mathf.Abs(height - _toastMainContent.RootRectTransform.anchoredPosition.y) < 1f ||
                (_toastMainContent.RootRectTransform.anchoredPosition.y > Toaster.TOAST_CONTAINER_HEIGHT &&
                    height > Toaster.TOAST_CONTAINER_HEIGHT)) {
                SetActualHeight(height);
                return;
            }

            SynthesisTween.MakeTween(_tweenKey, _toastMainContent.RootRectTransform.anchoredPosition.y, height, 0.2f,
