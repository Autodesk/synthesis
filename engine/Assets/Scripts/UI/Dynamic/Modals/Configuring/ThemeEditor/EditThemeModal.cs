...
namespace UI.Dynamic.Modals.Configuring.ThemeEditor
{
    ...
    public class EditThemeModal : ModalDynamic
    {
        ...
        private Dictionary<ColorManager.SynthesisColor, 
                (Color32 color, Content image, Content background, Label label)> _colors = new();
        ...
        private void CreateThemeSelection(Content content)
        {
            ...
            var selectThemeLabel = content.CreateLabel()
                .ApplyTemplate(VerticalLayout)
                .SetText("Select a Theme");

            var themeChooser = content
                .CreateDropdown()
                .ApplyTemplate(VerticalLayout)
            ...
            var deleteThemeButton = deleteContent.CreateButton()
                .ApplyTemplate(VerticalLayout)
                .StepIntoLabel(l => l.SetText("Delete Theme"))
                .AddOnClickedEvent(b => { DynamicUIManager.CreateModal<DeleteThemeModal>(); });
        }
        ...
        private void CreateColorSelection(Content content)
        {
            ...
            ColorManager.LoadedColors.ForEach(c =>
            {
                ...
                var button = colorContent.CreateButton().StepIntoLabel(l => l.RootGameObject.SetActive(false))
                    .AddOnClickedEvent(x => { SelectColor(c.Key); }).SetBackgroundColor<Button>(Color.clear)
                    .SetStretch<Button>().SetAnchoredPosition<Button>(Vector3.zero);
