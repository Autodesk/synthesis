...
namespace Synthesis.UI.Dynamic
{
    public class DownloadAssetModal : ModalDynamic
    {
        ...
        public override void Create()
        {
            ...
            robotScrollView = leftContent.CreateScrollView()
            ...
            fieldScrollView = rightContent.CreateScrollView()
                .SetHeight<ScrollView>(CONTENT_HEIGHT)
                .ApplyTemplate(VerticalLayout);
        ...
        {
            DownloadAsset(BASE_ASSET_URL + "Robots/" + robotName, AddRobotModal.ParsePath(Path.Combine("$appdata/Autodesk/Synthesis", "Mira", robotName), '/'));
            _downloadedRobots.Add(robotName);
        }

        private void DownloadField(string fieldName)
        {
            DownloadAsset(BASE_ASSET_URL + "Fields/" + fieldName, AddRobotModal.ParsePath(Path.Combine("$appdata/Autodesk/Synthesis", "Mira", "Fields", fieldName), '/'));
        ...
        private void CreateAssetEntries()
        {
            ...
            foreach (string robotName in robots)
            {
                ...
                var downloadButton = robotScrollView.Content.CreateLabeledButton()
                    ...
                    .StepIntoButton(b =>
                        b.StepIntoLabel(l => l.SetText("Download" + (alreadyDownloaded ? "ed" : ""))))
                    .ApplyTemplate(VerticalLayout);
