...
const SettingsModal: React.FC<ModalImplProps<void, void>> = ({ modal }) => {
	...
	useEffect(() => {
		...
		if (modal) {
			configureScreen(
				modal,
				{ title: "Settings", allowClickAway: false },
				{ onBeforeAccept: save, onCancel },
			);
		}
	}, [modal, save, configureScreen]);

	const writePreference = <K extends GlobalPreference>(
		pref: K,
	...
	return (
		...
		>
			...
			<StatefulSlider
				...
				onChange={(value) =>
					writePreference("ViewCubeRotationSensitivity", value)
				}
				step={0.06}
			...
			<Stack direction="column">
				...
				<Checkbox
					...
					checked={PreferencesSystem.getGlobalPreference(
						"RenderProtectedZones",
					)}
					onClick={(checked) =>
						writePreference("RenderProtectedZones", checked)
					}
