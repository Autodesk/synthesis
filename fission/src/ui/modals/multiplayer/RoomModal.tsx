...
import {MultiplayerInitProps} from "@/modals/multiplayer/MultiplayerStartModal.tsx";
...
const RoomModal: React.FC<RoomModalProps> = ({initialRoomList, url, startWorldCallback, onBack}) => {
    ...
    const updateRoomList = useCallback(async () => {
        ...
        }), "Refreshing rooms timed out", 5000).finally(() => setTimeout(() => setUpdatingRoomList(false), 150))
    }, [url])
    ...
    return (<Stack direction="column" gap={2} className="overflow-y-auto rounded-md p-2 min-w-[300px]">
        <Stack gap={0.5}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems={"center"}>
            ...
            <IconButton onClick={updateRoomList} sx={{animation: updatingRoomList ? "continuous-spin 1s linear infinite" : undefined}} color={"secondary"}><SynthesisIcons.REFRESH_LARGE/></IconButton>
        </Stack>
        {roomList.map((room) => (
            <Box
            ...
            >
                <Stack direction={"row"} justifyContent="space-between" gap={2}>
                    <Stack direction="column" gap={1}>
                        <Label size={"md"}>
                            {room.authority != null ? `${room.authority}'s Room` : "Empty Room"}
                        </Label>
                        <Label size={"sm"}>{room.id}</Label>
                    </Stack>
                    <Button disabled={joinDisabled || room.locked}
