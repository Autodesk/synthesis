...
const ImportLocalMirabufModal: React.FC<ModalImplProps> = ({ modal }) => {
	...
	useEffect(() => {
		...
		modal!.props.onAccept = async () => {
			if (selectedFile && miraType !== undefined) {
				...
				await MirabufCachingService.cacheAndGetLocalWithInfo(
					hashBuffer,
					miraType,
				)
					.then((result) => {
		...
		<Stack className="items-center" gap={5}>
			...
			{selectedFile && (
				<Typography
					...
					variant="h5"
				>{`Selected File: ${selectedFile.name}`}</Typography>
			)}
		</Stack>
	);
};
