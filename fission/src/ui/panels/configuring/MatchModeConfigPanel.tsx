import Panel, { PanelImplProps, PanelPropsImpl } from "@/ui/components/Panel";
import React, {
	ChangeEvent,
	useContext,
	useEffect,
...
const MatchModeConfigPanel: React.FC<PanelImplProps<void>> = ({
	panel,
	parent,
}) => {
		...
		const loadConfigs = () => {
			try {
				const defaultConfigs =
					DefaultMatchModeConfigs.defaultMatchModeConfigCopies;
				...
				const uniqueConfigsById = Array.from(
					new Map(combinedConfigs.map((item) => [item.id, item])).values(),
				);
						...
						secondaryOnClick={
							!config.isDefault
								? () => {
										...
										const updatedConfigs = matchModeConfigs.filter(
											(c) => c.id !== config.id,
										);
										setMatchModeConfigs(updatedConfigs);
										// Only save custom configs to local storage
										const customConfigs = updatedConfigs.filter(
											(c) => !c.isDefault,
										);
	...
	const validateAndNormalizeMatchModeConfig = (
		config: unknown,
	): MatchModeConfig | null => {
		let valid = true;

		// Type guard to check if config is an object
		if (typeof config !== "object" || config === null) {
			console.error(
				"Match mode config validation failed: config must be an object",
			);
			globalAddToast(
				"error",
		...
		const typeError = (id: string, expectedType?: string) => {
			const errorMessage = expectedType
				...
				: "is required";
			console.error(
				`Match mode config validation failed: the '${id}' field ${errorMessage}`,
			);
			globalAddToast(
				"error",
		...
		const normalizedConfig: MatchModeConfig = {
			...
			autonomousTime:
				typeof configObj.autonomousTime === "number"
					? configObj.autonomousTime
					: DEFAULT_AUTONOMOUS_TIME,
			teleopTime:
				typeof configObj.teleopTime === "number"
					? configObj.teleopTime
					: DEFAULT_TELEOP_TIME,
			endgameTime:
				typeof configObj.endgameTime === "number"
					? configObj.endgameTime
					: DEFAULT_ENDGAME_TIME,
		};
	...
	const handleFileUpload = async (file: File) => {
		...
		try {
			...
			const normalizedConfig =
				validateAndNormalizeMatchModeConfig(parsedConfig);
			if (!normalizedConfig) {
				globalAddToast(
					"error",
			...
			if (
				matchModeConfigs.find((config) => config.id === normalizedConfig.id)
			) {
				console.error(
			...
			// Only save custom configs to local storage
			const customConfigs = [
				...matchModeConfigs.filter((c) => !c.isDefault),
				normalizedConfig,
			];
			window.localStorage.setItem(
				"match-mode-configs",
