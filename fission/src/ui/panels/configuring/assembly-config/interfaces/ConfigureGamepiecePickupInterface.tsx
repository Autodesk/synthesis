...
function save(
    ...
    selectedRobot.intakePreferences.deltaTransformation = threeMatrix4Array(deltaTransformation)
...
const ConfigureGamepiecePickupInterface: React.FC<ConfigPickupProps> = ({ selectedRobot }) => {
    ...
    const placeholderMesh = useMemo(() => {
        const material = World.sceneRenderer.createToonMaterial(reactRgbaColorThreeColor(theme.HighlightHover.color))
    ...
    const gizmoComponent = useMemo(() => {
        if (selectedRobot?.intakePreferences) {
            const postGizmoCreation = (gizmo: GizmoSceneObject) => {
                ...
                const deltaTransformation = arrayThreeMatrix4(selectedRobot.intakePreferences!.deltaTransformation)
