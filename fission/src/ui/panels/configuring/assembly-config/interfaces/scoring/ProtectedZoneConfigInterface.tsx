...
function save(
	...
	const gizmoTransformation = new THREE.Matrix4().compose(
		translation,
		rotation,
		scale,
	);
	...
	const deltaTransformation = gizmoTransformation.premultiply(
		fieldTransformation.invert(),
	);
	...
	if (!field.fieldPreferences.protectedZones.includes(zone))
		field.fieldPreferences.protectedZones.push(zone);
...
const ZoneConfigInterface: React.FC<ZoneConfigProps> = ({
	selectedField,
	selectedZone,
	saveAllZones,
}) => {
	...
	const [selectedNode, setSelectedNode] = useState<RigidNodeId | undefined>(
		selectedZone.parentNode,
	);
	const [points, setPoints] = useState<number>(selectedZone.penaltyPoints);
	const [contactType, setContactType] = useState<ContactType>(
		selectedZone.contactType || ContactType.ROBOT_ENTERS,
	);
	const [activeDuring, setActiveDuring] = useState<MatchModeType[]>(
		selectedZone.activeDuring,
	);
		...
		if (selectedField && selectedZone) {
			const postGizmoCreation = (gizmo: GizmoSceneObject) => {
				...
				if (!nodeBodyId) {
					...
					nodeBodyId = selectedField.mechanism.nodeToBody.get(
						selectedField.rootNodeId,
					)!;
				}
				...
				const props = deltaFieldTransformsPhysicalProp(
					deltaTransformation,
					fieldTransformation,
				);

				gizmo.obj.position.set(
					props.translation.x,
					props.translation.y,
					props.translation.z,
				);
		...
		<Stack gap={2} className="bg-background-secondary rounded-md p-2">
			...
			<Button
				onClick={() => {
					...
					if (gizmoRef.current)
						(gizmoRef.current.obj as THREE.Mesh).material =
							alliance === "blue" ? redMaterial : blueMaterial;
				}}
			...
			<Select
				...
				value={contactType}
			>{CONTACT_TYPE_OPTIONS.map(opt => <MenuItem key={opt} value={opt}>{opt}</MenuItem>)}</Select>
