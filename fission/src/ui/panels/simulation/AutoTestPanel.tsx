...
import { joltMat44ThreeMatrix4, threeQuaternionJoltQuat, threeVector3JoltRVec3 } from "@/util/TypeConversions"
    ...
    World.sceneRenderer.sceneObjects.forEach(sceneObj => {
        if (sceneObj instanceof MirabufSceneObject) {
            sceneObj.mechanism.nodeToBody.forEach(bodyId => {
                ...
                joltMat44ThreeMatrix4(transform).decompose(translation, rotation, new THREE.Vector3(1, 1, 1))
                captures.push({
                    id: bodyId,
                    pos: threeVector3JoltRVec3(translation),
                    rot: threeQuaternionJoltQuat(rotation),
                })
            })
        }
    })
