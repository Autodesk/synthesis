...
const DeveloperToolPanel: React.FC<PanelPropsImpl> = ({ panelId }) => {
    ...
    return (
        ...
        >
            ...
            {!fieldLoaded && (
                <div className="text-red-600 m-4">No mira field loaded.</div>
            )}
            {editor && (
                <div className="flex flex-col md:flex-row gap-6 items-start">
                {/* Key List */}
                <div className="min-w-[220px] bg-gray-700 dark:bg-gray-800 rounded-lg p-3 shadow-sm flex flex-col gap-2">
                    <div className="font-bold text-base mb-1 text-gray-100">
                    Devtool Data Keys
                    </div>
                    <ul className="list-none p-0 m-0 flex-1">
                    {keys.length === 0 && (
                        <li className="text-gray-400 italic">No devtool data</li>
                    )}
                    {keys.map((key) => (
