...
class MirabufCachingService {
    ...
    /**
     * Overwrites an existing cache entry with a modified assembly, keeping the same hash key.
     * Used to persist in-editor devtool changes (e.g. scoring zones) without re-hashing.
     */
    public static async persistDevtoolChanges(
        ...
        try {
            ...
            this._inMemoryCache[cacheId] = buffer

            if (canOPFS) {
                const fileHandle = await fsHandle.getFileHandle(cacheId, { create: true })
                const writable = await fileHandle.createWritable()
