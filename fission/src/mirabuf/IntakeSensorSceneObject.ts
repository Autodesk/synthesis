...
class IntakeSensorSceneObject extends SceneObject {
    ...
    public setup(): void {
        if (this._parentAssembly.intakePreferences) {
            ...
            this._deltaTransformation = arrayThreeMatrix4(this._parentAssembly.intakePreferences.deltaTransformation)
    ...
    public update(): void {
        if (this._joltBodyId && this._parentBodyId && this._deltaTransformation) {
            ...
            const bodyTransform = this._deltaTransformation
                .clone()
                .premultiply(joltMat44ThreeMatrix4(parentBody.GetWorldTransform()))
            const position = new THREE.Vector3(0, 0, 0)
            const rotation = new THREE.Quaternion(0, 0, 0, 1)
            bodyTransform.decompose(position, rotation, new THREE.Vector3(1, 1, 1))

            World.physicsSystem.setBodyPosition(this._joltBodyId, threeVector3JoltRVec3(position))
            World.physicsSystem.setBodyRotation(this._joltBodyId, threeQuaternionJoltQuat(rotation))
