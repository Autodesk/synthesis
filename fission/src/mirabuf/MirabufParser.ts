...
import { mirabufTransformThreeMatrix } from "@/util/TypeConversions"
    ...
    private loadGlobalTransforms() {
        ...
        const getTransforms = (node: mirabuf.INode, parent: THREE.Matrix4) => {
            node.children!.forEach(child => {
                ...
                const mat = mirabufTransformThreeMatrix(partInstance.transform!)!
        ...
        root.children?.forEach(child => {
            ...
            const mat = partInstance.transform
                ? mirabufTransformThreeMatrix(partInstance.transform)
                : def.baseTransform
                  ? mirabufTransformThreeMatrix(def.baseTransform)
