...
describe("Mirabuf Parser Tests", () => {
    test("Generate Rigid Nodes (Dozer_v9.mira)", async () => {
        const spikeMira = await MirabufCachingService.CacheRemote("/api/mira/Robots/Dozer_v9.mira", MiraType.ROBOT)
    ...
    test("Generate Rigid Nodes (FRC_Field_2018_v14.mira)", async () => {
        const field = await MirabufCachingService.CacheRemote("/api/mira/Fields/FRC Field 2018_v13.mira", MiraType.FIELD)
            .then(x => MirabufCachingService.Get(x!.id, MiraType.ROBOT))
    ...
    test("Generate Rigid Nodes (Team_2471_(2018)_v7.mira)", async () => {
        const mm = await MirabufCachingService.CacheRemote("/api/mira/Robots/Team 2471 (2018)_v7.mira", MiraType.ROBOT)
            .then(x => MirabufCachingService.Get(x!.id, MiraType.ROBOT))
