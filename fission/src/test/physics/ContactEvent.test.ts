...
describe("Contact Event Integration Tests", () => {
    ...
    test("Contact validate callback is a no-op (PERF-005: removed event dispatch overhead)", () => {
        // OnContactValidate now returns AcceptAllContactsForThisBodyPair directly without
        // allocating events or queuing them, eliminating per-candidate-pair WASM→JS overhead.
