...
export async function getMiraAssembly(name: (keyof typeof ROBOT_MODELS|keyof typeof FIELD_MODELS)) {
    if (name in ROBOT_MODELS) {
        return await MirabufCachingService.cacheRemoteAndReturn(ROBOT_MODELS[name as keyof typeof ROBOT_MODELS], MiraType.ROBOT)
    } else {
        return await MirabufCachingService.cacheRemoteAndReturn(FIELD_MODELS[name as keyof typeof FIELD_MODELS], MiraType.FIELD)
    }
}
