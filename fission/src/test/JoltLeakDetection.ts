...
import { JOLT_OWNERSHIP_HINTS } from "@/test/JoltOwnershipHints.generated"
...
// One-line triage hint per unclassified class, from the jolt-ownership codegen pipeline's own
// findings (see JoltOwnershipHints.generated.ts's header) -- so the first response to a new
// failure here is "read this" instead of "go read glue.cpp and grep call sites yourself."
function formatHints(diffs: LeakDiff[]): string {
    return diffs
        .map(d => {
            const hint = JOLT_OWNERSHIP_HINTS[d.className]
            if (!hint) {
                return (
                    `  ${d.className}: not seen by the ownership tool at all -- either added to fission/src ` +
                    `after the last \`bun run jolt-ownership:generate\`, or reached via a call shape the ` +
                    `scanner doesn't cover yet ([Value] attribute field reads; see ` +
                    `fission/scripts/jolt-ownership/README.md). Try regenerating first.`
                )
            }
            const refTargetNote = hint.isRefTarget
                ? "IS RefTarget-derived (AddRef/Release/GetRefCount) -- an INTERNAL_REF-shaped return alone " +
                  "does NOT prove this class safe, since it may hide a real allocation inside a delegated " +
                  "Create() call (see Constraint's entry in JoltClassClassification.ts). Needs either a " +
                  "handoff-timing test (JoltHandoffTiming.test.ts) or a hand-attested manual entry."
                : "not RefTarget-derived"
            const destructorNote = hint.hasDestructor
                ? ""
                : " Has no generated __destroy__ binding at all -- JOLT.destroy() throws unconditionally " +
                  "for every instance of this class, independent of any call site."
            return (
                `  ${d.className}: previously classified ${hint.status} by the ownership tool, but that ` +
                `verdict doesn't cover this specific nonzero delta (a new call site, most likely). ${refTargetNote}.${destructorNote}\n` +
                `    known evidence: ${hint.evidence.join("; ") || "(none recorded)"}`
            )
        })
        .join("\n")
}
    ...
    if (unclassified.length > 0) {
        throw new Error(
            `unclassified class(es) had a nonzero delta, classify them before this check can pass ` +
                ...
                `Triage hints from the jolt-ownership tool:\n${formatHints(unclassified)}`
        )
    }
