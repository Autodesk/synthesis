...
import { SimGeneric } from "../../wpilib_brain/WPILibBrain"
...
class WPILibArcadeDriveBehavior extends Behavior {
    ...
    public Update(_: number): void {
        ...
        this.leftWheels.forEach(
            wheel =>
                (wheel.targetWheelSpeed =
                    SimGeneric.Get<number>(wheel.deviceType!, wheel.device!, "<percentOutput", 0)! * this._driveSpeed)
        )
        this.rightWheels.forEach(
            wheel =>
                (wheel.targetWheelSpeed =
                    SimGeneric.Get<number>(wheel.deviceType!, wheel.device!, "<percentOutput", 0)! * this._driveSpeed)
        )
