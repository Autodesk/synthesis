...
import { threeVector3JoltVec3, joltVec3ThreeVector3 } from "@/util/TypeConversions"
...
class DragModeSystem extends WorldSystem {
            ...
            if (association?.sceneObject && association.sceneObject instanceof MirabufSceneObject) {
                ...
                if (body) {
                    ...
                    if (!isStatic && !isFieldStructure) {
                        const hitPointVec = joltVec3ThreeVector3(hitResult.point)
    ...
    private raycastFromMouse(mousePos: [number, number]) {
        ...
        return World.physicsSystem.rayCast(threeVector3JoltVec3(origin), threeVector3JoltVec3(direction))
    }
    ...
    private updateDragForce(): void {
        ...
        if (distance > DragModeSystem.DRAG_FORCE_CONSTANTS.MINIMUM_DISTANCE_THRESHOLD) {
            ...
            const joltForce = threeVector3JoltVec3(forceNeeded)
            ...
            const joltTorque = threeVector3JoltVec3(torque)
