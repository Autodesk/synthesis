...
import MirabufSceneObject, {
	type RigidNodeAssociate,
} from "@/mirabuf/MirabufSceneObject";
...
import {
	type ContextData,
	ContextSupplierEvent,
} from "@/ui/components/ContextMenuData";
import { globalOpenPanel } from "@/ui/components/GlobalUIControls";
import {
	type PixelSpaceCoord,
	SceneOverlayEvent,
	SceneOverlayEventKey,
} from "@/ui/components/SceneOverlayEvents";
import {
	TouchControlsEvent,
	TouchControlsEventKeys,
} from "@/ui/components/TouchControls";
...
class SceneRenderer extends WorldSystem {
	...
	public filterSceneObjects<T extends SceneObject>(
		predicate: (obj: SceneObject) => obj is T,
	): T[] {
	...
	public readonly mirabufSceneObjects = {
		...
		findWhere: (
			predicate: Parameters<
				(typeof Array<MirabufSceneObject>)["prototype"]["find"]
			>[0],
		) => this.mirabufSceneObjects.getAll().find(predicate),
		getField: () =>
			this.mirabufSceneObjects.findWhere(
				(obj) => obj.miraType == MiraType.FIELD,
			),
		getRobots: () =>
			this.mirabufSceneObjects
				.getAll()
	...
	public constructor() {
		...
		// Orbit controls
		this._screenInteractionHandler = new ScreenInteractionHandler(
			this._renderer.domElement,
		);
		this._screenInteractionHandler.contextMenu = (e) => this.onContextMenu(e);

		this._cameraControls = new CustomOrbitControls(
			this._mainCamera,
			this._screenInteractionHandler,
		);
	}
	...
	public updateCanvasSize() {
		...
		this._mainCamera.aspect =
			window.innerHeight > 0 ? window.innerWidth / window.innerHeight : 1.0;
	...
	public changeLighting(fancyShadows: boolean): void {
		...
		const shadowMapSize = Math.min(
			graphicsSettings.shadowMapSize,
		...
		if (!fancyShadows) {
			...
			this._light = new THREE.DirectionalLight(
				0xffffff,
				graphicsSettings.lightIntensity,
			);
			...
			this._light.shadow.mapSize = new THREE.Vector2(
				shadowMapSize,
				shadowMapSize,
			);
		...
		this._light = new CSM({
			...
			customSplitsCallback: (
				...
				for (let i = 1; i < cascades; i++) {
					...
					const logarithmicFactor =
						(near * (far / near) ** (i / cascades)) / far;
					const combinedFactor =
						uniformFactor * (1 - blend) + logarithmicFactor * blend;
	...
	public worldToPixelSpace(world: THREE.Vector3): PixelSpaceCoord {
		...
		return [
			(window.innerWidth * (screenSpace.x + 1.0)) / 2.0,
			(window.innerHeight * (1.0 - screenSpace.y)) / 2.0,
		];
	}
	...
	public isAnyGizmoDragging(): boolean {
		return [...this._gizmosOnMirabuf.values()].some(
			(obj) => obj.gizmo.dragging,
		);
	}
