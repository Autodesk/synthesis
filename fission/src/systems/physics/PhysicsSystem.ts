import {
    joltRVec3JoltVec3,
    joltVec3JoltRVec3,
    mirabufFloatArrJoltFloat3,
    mirabufFloatArrJoltVec3,
    mirabufVector3JoltRVec3,
    mirabufVector3JoltVec3,
...
class PhysicsSystem extends WorldSystem {
    ...
    public createBox(
        ...
        const size = threeVector3JoltVec3(halfExtents)
        const shape = new JOLT.BoxShape(size, 0.1)
        JOLT.destroy(size)

        const pos = position ? threeVector3JoltRVec3(position) : new JOLT.RVec3(0.0, 0.0, 0.0)
        const rot = joltQuat(rotation)
    ...
    private createHingeConstraint(
        ...
        const jointOrigin = jointDefinition.origin
            ? mirabufVector3JoltRVec3(jointDefinition.origin as mirabuf.Vector3)
            : new JOLT.RVec3(0, 0, 0)
        // TODO: Offset transformation for robot builder.
        const jointOriginOffset = jointInstance.offset
            ? mirabufVector3JoltRVec3(jointInstance.offset as mirabuf.Vector3)
            : new JOLT.RVec3(0, 0, 0)
    ...
    public createWheelConstraint(
        ...
        wheelSettings.mPosition = joltRVec3JoltVec3(anchorPoint.AddRVec3(axis.Mul(0.1)))
    ...
    private createBallConstraint(
        ...
        const jointOrigin = jointDefinition.origin
            ? mirabufVector3JoltVec3(jointDefinition.origin as mirabuf.Vector3)
            : new JOLT.Vec3(0, 0, 0)
        // TODO: Offset transformation for robot builder.
        const jointOriginOffset = jointInstance.offset
            ? mirabufVector3JoltVec3(jointInstance.offset as mirabuf.Vector3)
            : new JOLT.Vec3(0, 0, 0)
        ...
        for (let i = 0; i < constraints.length; ++i) {
            ...
            hingeSettings.mPoint1 = hingeSettings.mPoint2 = joltVec3JoltRVec3(anchorPoint)
    ...
    public createBodiesFromParser(parser: MirabufParser, layerReserve?: LayerReserve): Map<string, Jolt.BodyID> {
        ...
        nonPhysicsNodes.forEach(rn => {
            ...
            rn.parts.forEach(partId => {
                ...
                const transform = threeMatrix4JoltMat44(parser.globalTransforms.get(partId)!)
    ...
    private createConvexShapeSettingsFromPart(
        ...
        partDefinition.bodies!.forEach(body => {
            ...
            for (let i = 0; i < verts.length; i += 3) {
                const vert = mirabufFloatArrJoltVec3(verts, i)
    ...
    private createConcaveShapeSettingsFromPart(
        ...
        partDefinition.bodies!.forEach(body => {
            ...
            for (let i = 0; i < vertArr.length; i += 3) {
                const vert = mirabufFloatArrJoltFloat3(vertArr, i)
    ...
    public rayCast(from: Jolt.Vec3, dir: Jolt.Vec3, ...ignoreBodies: Jolt.BodyID[]): RayCastHit | undefined {
        const ray = new JOLT.RRayCast(joltVec3JoltRVec3(from), dir)
        ...
        return { data: collector.mHit, point: joltRVec3JoltVec3(hitPoint), ray: ray }
    }
