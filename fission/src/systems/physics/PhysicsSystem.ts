...
export type DOFSpecs = Omit<mirabuf.joint.IDOF, "name" | "pivotDirection" | "dynamics" | "axis"> & {
    friction: number
    axis: Jolt.Vec3
}
...
class PhysicsSystem extends WorldSystem {
    ...
    public createJointsFromParser(parser: MirabufParser, mechanism: Mechanism) {
        ...
        joints.forEach(([jointGuid, jointInst]) => {
            ...
            const prefMotors = PreferencesSystem.getRobotPreferences(parser.assemblyId).motors
            ...
            switch (jDef.jointMotionType!) {
                case mirabuf.joint.JointMotion.REVOLUTE: {
                    if (isWheel(jDef)) {
                        const preferences = PreferencesSystem.getRobotPreferences(parser.assemblyId)
    ...
    private createHingeConstraint(
        ...
        setAxes(rotationalFreedom, hingeConstraintSettings, versionNum)
        applyHingeLimits(rotationalFreedom, hingeConstraintSettings)
    ...
    private createSliderConstraint(
        ...
        setAxes(freedom, constraintSettings)
        applySliderLimits(freedom, constraintSettings)
    ...
    private createBallConstraint(
        ...
        const newGhostBody = () => {
            const gb = this.createGhostBody(anchorPoint, false)

            this._joltBodyInterface.AddBody(gb.GetID(), JOLT.EActivation_Activate)
            mechanism.ghostBodies.push(gb.GetID())
    ...
    public destroyBodies(...bodies: Jolt.Body[]) {
        this.destroyBodyIds(...bodies.map(x => x.GetID()))
    }
    ...
    public destroyBodyIds(...bodies: Jolt.BodyID[]) {
        ...
        bodies.forEach((x, i) => {
            ...
            this.releaseBodyShapeByKey(keys[i])
        })
    }
    ...
    public destroyMechanism(mech: Mechanism) {
        ...
        // Prune mechanism bodies from `this._bodies` before destroying them so later bulk
        // cleanup does not re-destroy the same freed bodies.
        const mechBodyKeys = new Set(
            [...mech.nodeToBody.values(), ...mech.ghostBodies].map(x => x.GetIndexAndSequenceNumber())
        )
        this._bodies = this._bodies.filter(x => !mechBodyKeys.has(x.GetIndexAndSequenceNumber()))

        this.unregisterSphereGamePieceBodies([...mech.nodeToBody.values()])
        mech.nodeToBody.forEach(x => {
            // Captured before DestroyBody — see destroyBodyIds's comment.
            const key = x.GetIndexAndSequenceNumber()
            this._joltBodyInterface.RemoveBody(x)
            this._joltBodyInterface.DestroyBody(x)
            this.releaseBodyShapeByKey(key)
        })
...
function setupCollisionFiltering(settings: Jolt.JoltSettings) {
    // TODO
    // Ensure that `objectFilter` is freed by `settings` (`settings.mObjectLayerPairFilter`)
