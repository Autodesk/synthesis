	...
	static ernie: InputSupplier = () => {
		...
		return {
			...
			inputs: [
				...
				AxisInput.onKeyboardSingleKey(
					'joint 1',
					'Digit1',
					negativeModifierKeys
				),
				AxisInput.onKeyboardSingleKey(
					'joint 2',
					'Digit2',
	...
	public static newBlankScheme(drivetype: DriveType): InputScheme {
		...
		switch (drivetype) {
			case DriveType.ARCADE:
				driveInputs = [
					AxisInput.unbound('arcadeDrive'),
					AxisInput.unbound('arcadeTurn'),
				];
				break;
			case DriveType.TANK:
				driveInputs = [
					AxisInput.unbound('tankLeft'),
					AxisInput.unbound('tankRight'),
				];
				break;
		}
		return {
