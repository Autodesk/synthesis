...
class InputSchemeManager {
	...
	public static get customInputSchemes(): InputScheme[] {
		...
		this._customSchemes =
			PreferencesSystem.getGlobalPreference('InputSchemes');
	...
	public static get allInputSchemes(): InputScheme[] {
		...
		this.defaultInputSchemes.forEach((defaultScheme) => {
			if (
				allSchemes.some(
					(s) => s.schemeName === defaultScheme.schemeName
				)
			)
				return;
	...
	public static availableInputSchemesByType(
		...
		return allSchemes.filter((entry) =>
			entry.scheme.supportedDrivetrains.includes(driveType)
		);
	}

	/** Creates an array of every input scheme that is not currently in use by a robot */
	public static availableInputSchemesByBrain(
		brainIndex: number
	): InputSchemeAvailability[] {
		const driveType =
			SynthesisBrain.brainIndexMap.get(brainIndex)?.driveType;
