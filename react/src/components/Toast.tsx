...
const Toast: React.FC<ToastData> = ({ id, type, title, description }) => {
    ...
    switch (type) {
        case 'info':
            icon = <BsFillWrenchAdjustableCircleFill size={48} className="h-full w-full text-white" />
            className = "bg-purple-700";
            break;
        case 'warning':
            icon = <AiFillWarning size={48} className="h-full w-full text-white" />
            className = "bg-yellow-500"
            break;
        case 'error':
            icon = <BiSolidErrorCircle size={48} className="h-full w-full text-white" />
