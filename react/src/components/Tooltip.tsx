            ...
            <div className="absolute flex flex-col gap-1 px-8 pt-2 pb-4 rounded-lg left-1/2 -translate-x-1/2 top-2 bg-background">
                ...
                {controls?.map(c => (
                    <Stack direction={StackDirection.Horizontal} key={`${c.control}`} spacing={8} justify="around" className="min-w-max">
                        <Label size={LabelSize.Small} className="bg-background-secondary align-middle px-2 rounded-md min-w-fit">{c.control}</Label>
                        <Label size={LabelSize.Small} className="min-w-fit align-middle">{c.description}</Label>
                    </Stack>
                ))}
            </div>
        )
