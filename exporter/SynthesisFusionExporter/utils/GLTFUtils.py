...
def split_list_by_data_type(l: list, data_type: GLTFConstants.DataType):
    """
    ...
    """
    if not (len(l) % GLTFConstants.DataType.num_elements(data_type) == 0):
        raise ValueError("List length does not match specified data type")
    num_elements = GLTFConstants.DataType.num_elements(data_type)
    return [l[i:i + num_elements] for i in range(0, len(l), num_elements)]


def max_components(l: list, data_type: GLTFConstants.DataType) -> list:
    """
    ...
    result = [-math.inf] * GLTFConstants.DataType.num_elements(data_type)
...
def min_components(l: list, data_type: GLTFConstants.DataType) -> list:
    """
