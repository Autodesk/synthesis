using System;
using SynthesisExporterInventor.RigidAnalyzer;
using SynthesisExporterInventor.Utilities;
using Inventor;

namespace SynthesisExporterInventor.SkeletalStructure
{
    public class RotationalJoint : RotationalJoint_Base, InventorSkeletalJoint
    {
        ...
        public void ReloadInventorJoint()
        {
            try
            {
                axis = MathUtils.ToBXDVector(wrapped.rigidJoint.geomOne.Normal);
                basePoint = MathUtils.ToBXDVector(wrapped.rigidJoint.geomOne.Center);
            }
            catch
            {
