...
def createTextGraphics(wheel: adsk.fusion.Occurrence, _wheels) -> None:
    try:
        ...
        if design:
            ...
            coords = adsk.fusion.CustomGraphicsCoordinates.create(indexPairs)
            line = graphics.addLines(
                coords,
                [],
                False,
            )
            line.color = adsk.fusion.CustomGraphicsShowThroughColorEffect.create(
                adsk.core.Color.create(0, 255, 0, 255), 0.2
            )  # bright-green color
            line.weight = 1
