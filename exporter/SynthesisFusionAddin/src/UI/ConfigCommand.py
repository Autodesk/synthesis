        ...
        try:
            ...
            if type(saved) == str:
                try:
                    ...
                    previous = json.loads(saved, object_hook=lambda d: SimpleNamespace(**d))
                except:
                    self.log.error("Failed:\n{}".format(traceback.format_exc()))
                    gm.ui.messageBox("Failed to read previous Unity Configuration\n  - Using default configuration")
            ...
            removeWheelInput = wheel_inputs.addBoolValueInput("wheel_delete", "Remove", False)  # remove button
            ...
            wheelTableInput.addCommandInput(  # create textbox input using helper (component name)
                self.createTextBoxInput("name_header", "Name", wheel_inputs, "Joint name", bold=False),
                0,
                1,
            )
            ...
            removeJointInput = joint_inputs.addBoolValueInput("joint_delete", "Remove", False)  # remove button
            ...
            for joint in list(gm.app.activeDocument.design.rootComponent.allJoints) + list(
...
class ConfigureCommandExecuteHandler(adsk.core.CommandEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            mode_dropdown = eventArgs.command.commandInputs.itemById("general_settings").children.itemById("mode")
            ...
            if platform.system() == "Windows":
                if isRobot:
                    ...
                    savepath = (
                        os.getenv("APPDATA") + "\\Autodesk\\Synthesis\\Mira\\Fields\\" + processedFileName + ".mira"
                    )
            else:
                ...
                if isRobot:
                    if export_as_part_boolean.value:
                        savepath = home + "/.config/Autodesk/Synthesis/MixAndMatch/Mira/" + processedFileName + ".mira"
                    else:
                        savepath = home + "/.config/Autodesk/Synthesis/Mira/" + processedFileName + ".mira"
                else:
                    savepath = home + "/.config/Autodesk/Synthesis/Mira/Fields/" + processedFileName + ".mira"
