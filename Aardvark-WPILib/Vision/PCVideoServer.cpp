...
PCVideoServer::PCVideoServer()
	: m_serverTask("PCVideoServer", s_ServerTask)
	, m_newImageSem ()
	, m_stopServer (false)
{
...
*/
DWORD WINAPI PCVideoServer::s_ServerTask(LPVOID thisPtr)
{
	return ((PCVideoServer*)thisPtr)->ServerTask();
}
...
int PCVideoServer::ServerTask()
{
	...
	//while (true)
	//{
	//	// API equiv for windows? taskSafe();
	//	// Create the socket.
	...
	//		int headerSend = send(newPCSock, const_cast<char*>(header), 4,0);

	//		// Write image length to PC
	//		int lengthSend = send(newPCSock, reinterpret_cast<char*>(&numBytes), 4,0);

	//		// Write image to PC
	//		int sent = send (newPCSock, imageData, numBytes,0);
	...
	//	close (pcSock);
	//	pcSock = ERROR;
	//	// API equiv for windows? taskUnsafe();
	//	Wait(0.1);
	//}
	//return (OK);
