...
void Relay::InitRelay (uint8_t moduleNumber)
{
	...
	{
		sprintf(buf, "Relay Channel %lu", m_channel);
	...
	{
		sprintf(buf, "Forward Relay %lu (Module: %d)", m_channel, moduleNumber);
	...
	{
		sprintf(buf, "Reverse Relay %lu (Module: %d)", m_channel, moduleNumber);
