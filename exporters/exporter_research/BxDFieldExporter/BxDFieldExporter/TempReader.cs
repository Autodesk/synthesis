...
namespace ExportProcess
{
    public class TempReader
    {
                ...
                foreach (ComponentOccurrence component in currentDocument.ComponentDefinition.Occurrences)
                {
                    if (fileName.Replace("\\", "").Equals(NameFilter(component.Name)))
                    {
                        ...
                        for (int x = 1; x != 5; x++)
                        {
                            for (int y = 1; y != 5; y++)
                            {
                                trans[x-1, y-1] = (float)m.Cell[x, y];
                            }
                        }
                    }
                }
        ...
        private byte[] ProcessFieldData(FieldDataComponent fieldComponent)
        {
            ...
            fieldDataBytes.AddRange(BitConverter.GetBytes(fieldComponent.ID));
            ...
            byte[] fieldLength = BitConverter.GetBytes(fieldDataBytes.Count);
            fieldDataBytes.InsertRange(0, fieldLength);
        ...
        public byte[] ReadFiles()
        {
            try
            {
                ...
                foreach (FieldDataComponent fieldComponent in fieldComponents) bytesOfFiles.AddRange(ProcessFieldData(fieldComponent));
