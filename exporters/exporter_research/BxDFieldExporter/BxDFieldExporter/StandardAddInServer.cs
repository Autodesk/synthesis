    ...
    public class StandardAddInServer : Inventor.ApplicationAddInServer {
            ...
            if (SelectionDevice == SelectionDeviceEnum.kGraphicsWindowSelection && inExportView) {// if the selection is from the graphical interface and the exporter is active
                foreach (Object sel in JustSelectedEntities) {//looks at all things selected
                    if (sel is ComponentOccurrence) {// react only if sel is a part/ assembly
                        foreach (FieldDataComponent Component in FieldComponents) {// looks at all the components of parts
                            foreach (ComponentOccurrence occ in Component.compOcc) {// looks at all the occurences in the Components items
                                if (((ComponentOccurrence)sel).Equals(occ)) {// if the occurence is contained by anyof the components then react
                                    foreach (BrowserNode n in oPane.TopNode.BrowserNodes) {// looks at all the browser nodes in the top node
                                        if (n.BrowserNodeDefinition.Equals(Component.node)) {// if the browsernode is the same as the Components node then react
                        ...
                        foreach (UserCoordinateSystem ucs in ((AssemblyComponentDefinition)((AssemblyDocument)m_inventorApplication.ActiveDocument).ComponentDefinition).UserCoordinateSystems) {
                            foreach (BrowserNode n in oPane.TopNode.BrowserNodes) {// looks at all the browser nodes in the top node
                                if (n.BrowserNodeDefinition.Label.Equals(((UserCoordinateSystem)sel).Name)) {// if the browsernode is the same as the Components node then react
                                    n.DoSelect();// select the proper node
                                }
                            }
                        }
                ...
                foreach (Object sel in JustSelectedEntities) {//looks at all things selected
                    if (sel is BrowserNodeDefinition) {// react only if sel is a browsernodedef
                        foreach (FieldDataComponent f in FieldComponents) {// looks at all the components of parts
                            if (f.same(((BrowserNodeDefinition)sel))) {// if the browsernode is the same as a the Component's node
                                selectedComponent = f;// set the selected Component for the rest of the code to interact with
                                foreach (ComponentOccurrence o in selectedComponent.compOcc) {// looks at the occurences in the selected Component's part list
        ...
        private static void OnTimedEvent(object source, ElapsedEventArgs e) {
            ...
            foreach (BrowserNode node in oPane.TopNode.BrowserNodes) {// looks through all the nodes under the top node
                if (node.Selected) {// if the node is seleted
                    foreach (FieldDataComponent t in FieldComponents) {// looks at all the components in the fieldComponents
                        if (t.same(node.BrowserNodeDefinition)) {// if t is part of that browser node
                            if (!currentSelected.Equals(t)) {// is the selected node is no longer selected 
                                ...
                                foreach (ComponentOccurrence io in t.compOcc) {// looks at the occurences that are part of the component
                    ...
                    foreach (UserCoordinateSystem ucs in SpawnPoints) {
                        if (ucs.Name.Equals(node.BrowserNodeDefinition.Label)) {// if t is part of that browser node
                            if (!currentSelected.Equals(ucs)) {// is the selected node is no longer selected 
                                found = true;// tell the program it found the node
                                oSet.Clear();// clear the highlighted set in prep to add new occurrences
        ...
        public static BrowserNodeDefinition addComponent(String name) {
            ...
            try {
                ...
                foreach (FieldDataComponent Component in FieldComponents) {// look at all the fielddata Components in data Components
                    if (Component.Name.Equals(name)) {// check to see if it is the same
                        same = true;// if it is then tell the code
                    }
                }
                if (!same) {// if there is no duplicate name then add the Component
        ...
        public void cancleExporter_OnExecute(Inventor.NameValueMap Context) {
            try {
                ...
                foreach (FieldDataComponent data in FieldComponents) {// looks at all the components in fieldComponent
        ...
        private void readSaveFieldData() {
            try {
                ...
                foreach (String n in names) {// looks at the strings in names, each one represents a potential data Component
        ...
        private void writeFieldComponentNames() {
            ...
            foreach (FieldDataComponent Component in FieldComponents) {// looks at all the browser node under the top node
