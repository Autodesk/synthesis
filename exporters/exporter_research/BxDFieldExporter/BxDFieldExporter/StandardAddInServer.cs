    ...
    public class StandardAddInServer : Inventor.ApplicationAddInServer {
        ...
        public void Activate(Inventor.ApplicationAddInSite addInSiteObject, bool firstTime) {
            ...
            try {
                ...
                partRibbon = m_inventorApplication.UserInterfaceManager.Ribbons["Assembly"];// Get the assembly ribbon.
                partTab = partRibbon.RibbonTabs.Add("Field Exporter", "BxD:FieldExporter", "{e50be244-9f7b-4b94-8f87-8224faba8ca1}");// Get the "Part" tab.
                partPanel = partTab.RibbonPanels.Add("Exporter Control", "BxD:FieldExporter:ExporterControl", "{e50be244-9f7b-4b94-8f87-8224faba8ca1}");// inits the part panels
                ...
                editType = controlDefs.AddButtonDefinition("Edit type properties", "InventorAddInBrowserPaneAttempt5:EditTypeProperties", CommandTypesEnum.kNonShapeEditCmdType, m_ClientId, null, null);// init the button
        ...
        public void startExport_OnExecute(Inventor.NameValueMap Context) {
            ...
            try {
                if (!inExportView) {
                    ...
                    try {// if no browser pane previously created then create a new one
        ...
        private void oUIEvents_OnSelect(ObjectsEnumerator JustSelectedEntities, ref ObjectCollection MoreSelectedEntities, SelectionDeviceEnum SelectionDevice, Inventor.Point ModelPosition, Point2d ViewPosition, Inventor.View View) {
            ...
            if (SelectionDevice == SelectionDeviceEnum.kGraphicsWindowSelection && inExportView) {// if the selection is from the graphical interface and the exporter is active
                foreach (Object sel in JustSelectedEntities) {//looks at all things selected
                    if (sel is ComponentOccurrence) {// react only if sel is a part/ assembly
                        foreach (FieldDataType type in FieldTypes) {// looks at all the groups of parts
                            foreach (ComponentOccurrence occ in type.compOcc) {// looks at all the occurences in the types items
                                if (((ComponentOccurrence)sel).Equals(occ)) {// if the occurence is contained by anyof the groups then react
                                    foreach (BrowserNode n in oPane.TopNode.BrowserNodes) {// looks at all the browser nodes in the top node
                                        if (n.BrowserNodeDefinition.Equals(type.node)) {// if the browsernode is the same as the types node then react
            ...
            else if (SelectionDevice == SelectionDeviceEnum.kBrowserSelection && inExportView) {// if the selection is from the browser and the exporter is active, cool feature is that browsernode.DoSelect() calls this so I do all the reactions in here
                foreach (Object sel in JustSelectedEntities) {//looks at all things selected
                    if (sel is BrowserNodeDefinition) {// react only if sel is a browsernodedef
                        foreach (FieldDataType f in FieldTypes) {// looks at all the groups of parts
                            if (f.same(((BrowserNodeDefinition)sel))) {// if the browsernode is the same as a the type's node
                                selectedType = f;// set the selected type for the rest of the code to interact with
                                foreach (ComponentOccurrence o in selectedType.compOcc) {// looks at the occurences in the selected type's part list
        ...
        private static void OnTimedEvent(object source, ElapsedEventArgs e) {
            found = false;
            foreach (BrowserNode node in oPane.TopNode.BrowserNodes) {// looks through all the nodes under the top node
                if (node.Selected) {// if the node is seleted
                    foreach (FieldDataType t in FieldTypes) {// looks at all the groups in the fieldtypes
                        if (t.same(node.BrowserNodeDefinition)) {// if t is part of that browser node
                            if (!currentSelected.Equals(t)) {// is the selected node is no longer selected 
                                found = true;// tell the program it found the node
                                oSet.Clear();// clear the highlighted set in prep to add new occurrences
                                foreach (ComponentOccurrence io in t.compOcc) {// looks at the occurences that are part of the group
            ...
            if (!found) {// if the program didn't find any selected node then assume that the user deselected 
        ...
        public static BrowserNodeDefinition addType(string name) {
            ...
            try {
                ...
                foreach (FieldDataType type in FieldTypes) {// look at all the fielddata types in data types
                    if (type.Name.Equals(name)) {// check to see if it is the same
                        same = true;// if it is then tell the code
                    }
                }
                if (!same) {// if there is no duplicate name then add the type
                    ...
                    catch (Exception) {// if the method fails then assume that there is already a ClientNodeResource
                ...
                else {// if there is already something with the name
        ...
        public void cancleExporter_OnExecute(Inventor.NameValueMap Context) {
            try {
                ...
                foreach (FieldDataType data in FieldTypes) {// looks at all the groups in fieldtype
                    writeSave(data);// writes the saved data to the property set
                }
                FieldTypes = new ArrayList();// clear the arraylist of groups
                foreach (BrowserNode node in oPane.TopNode.BrowserNodes) {// looks at all the nodes under the top node
        ...
        private void readSave() {
            try {
                ...
                foreach (PropertySet s in sets) {// looks at all the properties in the propertysets
                    if (s.DisplayName.Equals("Number of Folders")) {// is the name is correct the assume it is what we are looking fos
                ...
                foreach (string n in names) {// looks at the strings in names, each one represents a potential data type
                    if (!n.Equals("")) {// splitting the string creates empty string where the limiter was before, this deals with them
                        foreach (PropertySet set in sets) {// looks at all the sets in the propertysets of the document
                            if (set.Name.Equals(n)) {// if the set name is the same as the name then we assume they are the same
                                ...
                                foreach (string m in keys) {// looks at the strings in names, each one represents a potential occurence
                                    if (!m.Equals("")) {// splitting the string creates empty string where the limiter was before, this deals with them
                                        resultObj = null;
                                        ...
                                        if (m_inventorApplication.ActiveDocument.ReferenceKeyManager.CanBindKeyToObject(refKey, 0, out resultObj, out context)) {// if the key can be bound then bind it
        ...
        private void writeBrowserFolderNames() {
            ...
            foreach (BrowserNode node in oPane.TopNode.BrowserNodes) {// looks at all the browser node under the top node
            ...
            catch (Exception) {// if that fails we assume that there is already that property set
                foreach (PropertySet s in sets) {// looks at all the property sets in the propertsets
                    if (s.DisplayName.Equals("Number of Folders")) {// if the name is the same then we assume that the property set is the same
            ...
            catch (Exception) {// if that fails we assume that there is already that property so we write to it
        ...
        private void writeSave(FieldDataType f) {
            ...
            catch (Exception) {// if that fails then we assume there is already a property set for this set
                foreach (PropertySet s in sets) {// looks at the property set in the document's property sets
            ...
            try {
                foreach (ComponentOccurrence n in f.compOcc) {// looks at all the occurences
            ...
            try {// try to write the data to new properties
            ...
            catch (Exception) {// if there is an error then assume that the properties are already there and in that case write the new values
