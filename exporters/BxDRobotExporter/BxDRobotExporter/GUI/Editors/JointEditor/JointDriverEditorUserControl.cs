...
namespace BxDRobotExporter.GUI.Editors.JointEditor
{
    public partial class JointDriverEditorUserControl : UserControl // TODO: Use this UserControl in the DriveChooser form and remove the save button in DriveChooser
    {
        ...
        private void EnableLiveSave(Control control)
        {
            ...
            AnalyticsUtils.LogEvent("Joint Card Editor", "System", "EnableLiveSave");
        }
        ...
        public void LoadValues() // TODO: Settings saving and loading should be done in a dedicated class
        {
            ...
            UnitBox.SelectedIndex = robotDataManager.RobotPreferMetric ? 1 : 0;
            WeightBox.Value = (decimal) (Math.Max(joint.weight, 0) * (robotDataManager.RobotPreferMetric ? 1 : 2.20462f)); // TODO: Re-use existing weight code
                    ...
                    switch (joint.cDriver.motor) // TODO: Use motor definition file
                    {
                        case MotorType.GENERIC:
                            RobotCompetitionDropDown.SelectedItem =
                                RobotExporterAddInServer.Instance.AddInSettingsManager.DefaultRobotCompetition.ToString();
        ...
        {
            AnalyticsUtils.LogEvent("Joint Card Editor", "System", "SaveChanges");
            if (disableAutoSave)
            {
                return;
            }
            ...
            else
            {
                ...
                if (cType.IsMotor())
                {
                    ...
                    RobotExporterAddInServer.Instance.AddInSettingsManager.DefaultRobotCompetition =
        ...
        {
            AnalyticsUtils.LogEvent("Joint Card Editor", "Option", "WheelType", cmbWheelType.SelectedIndex);
        ...
        {
            AnalyticsUtils.LogEvent("Joint Card Editor", "Option", "Competetion", RobotCompetitionDropDown.SelectedIndex);
        ...
        {
            AnalyticsUtils.LogEvent("Joint Card Editor", "Option", "CAN", rbCAN.Checked ? 0 : 1);
            UpdateLayout();
        }

        private void chkBoxDriveWheel_CheckedChanged(object sender, EventArgs e)
        {
            AnalyticsUtils.LogEvent("Joint Card Editor", "Option", "Drive Wheel", chkBoxDriveWheel.Checked ? 0 : 1);
            UpdateLayout();
        }
        
        private void cmbJointDriver_SelectedIndexChanged(object sender, EventArgs e)
        {
            AnalyticsUtils.LogEvent("Joint Card Editor", "Option", "Joint Driver", cmbJointDriver.SelectedIndex);
            UpdateLayout();
        }

        private void CalculatedWeightCheck_CheckedChanged(object sender, EventArgs e)
        {
            AnalyticsUtils.LogEvent("Joint Card Editor", "Option", "Calculate Weight", CalculatedWeightCheck.Checked ? 0 : 1) ;
            UpdateLayout();
        }
    }
}