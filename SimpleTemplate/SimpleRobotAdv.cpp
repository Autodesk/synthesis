...
private:
	...
	Talon shoot;
public:
	RobotDemo(void) : stick(1), stick2(2), drive(driveLeft, driveRight), mainRot(mainRotation), 
		mainRotPot(1,2), collect(collector), latch(latchA, latchB), shoot(shooter)
	{
		drive.SetSafetyEnabled(false);
		mainRotPot.SetVoltageForPID(false);
		printf("[ROBOT] Skunkworks code started.\n");
	}
	...
	void OperatorControl(void)
	{
		...
		while (IsOperatorControl() && IsEnabled()) 
		{
			...
			drive.ArcadeDrive(stick.GetY()/1.5+stick2.GetY()/1.5, -stick.GetX()/1.5 + -stick2.GetX()/1.5);
			collect.Set((stick2.GetRawButton(1) ? -1.0 : 0) + (stick.GetRawButton(1) ? -1.0 : 0));
