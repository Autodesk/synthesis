...
block_progress_bar 50

read -r -p "Would you like to install the Code Emulator as well? [ Y / N ] " response
if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]
then
	echo "Creating Emulator Directory..."
	sudo mkdir -p $HOME/.config/Autodesk/Synthesis/Emulator
	draw_progress_bar 60
	echo "Copying Emulator Files..."
	rsync --info=progress2 Emulator/zImage $HOME/.config/Autodesk/Synthesis/Emulator
	draw_progress_bar 70
	rsync --info=progress2 Emulator/rootfs.ext4 $HOME/.config/Autodesk/Synthesis/Emulator
	draw_progress_bar 80
	rsync --info=progress2 Emulator/zynq-zed.dtb $HOME/.config/Autodesk/Synthesis/Emulator
